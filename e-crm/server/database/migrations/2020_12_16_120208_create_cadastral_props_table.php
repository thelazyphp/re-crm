...
class CreateCadastralPropsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('cadastral_props', function (Blueprint $table) {
            $table->id();
            $table->foreignId('type_id')->index()->nullable()->comment('Тип недвижимого имущества');
            $table->string('inventory_number')->nullable()->comment('Инвентарный номер');
            $table->foreignId('function_id')->index()->nullable()->comment('Назначение');
    ...
    {
        Schema::dropIfExists('cadastral_props');
    }
}
