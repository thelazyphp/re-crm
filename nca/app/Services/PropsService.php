...
{
    /**
     * @param  \Illuminate\Http\UploadedFile  $props
     * @return void
     */
    public function import(UploadedFile $props)
    {
        $xlsx = \SimpleXLSX::parseData($props->get());
        $rows = $xlsx->rows();
        $offset = 0;
        ...
        foreach ($rows as $row) {
            ...
            if (isset($row[22])) {
                $attributes['proportion_before_transaction'] = str_replace(
                    ['{', '}'], '', trim($row[22])
                );
            }

            if (isset($row[23])) {
                $attributes['proportion_after_transaction'] = str_replace(
                    ['{', '}'], '', trim($row[23])
                );
            }
            ...
            try {
                ...
                if (isset($row[3])) {
                    if ($address = $geocoder->translateAddress(trim($row[3]))) {
                ...
                if (! $prop->flags) {
                    ...
                    if ($prop->entry_date) {
                        $curYear = now()->year;
                        $propYear = (new Carbon($prop->entry_date))->year;

                        if ($curYear - $propYear <= 2) {
                            $flags->new_building = true;
                        }
                    }

                    $prop->flags()->save($flags);
                }
            } catch (Throwable $e) {
                //
